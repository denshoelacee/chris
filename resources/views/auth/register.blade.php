...
#successMessage.hide {
    ...
    <div class="my-10 mr-10 w-full md:flex sm:flex-wrap">
        ...
        <div class="bg-[#F1F1F1] shadow-md rounded-lg md:m-10 pt-5 mr-10 p-5 w-full max-w-md min-h-[500px]">
            ...
            @if(session('success'))
            @endif
            <div id="successMessage" class="bg-green-200 border-green-600 text-green-600 border-l-4 p-4 fixed bottom-5 left-1      z-50 opacity-0 transition-opacity duration-500" role="alert">
                <p class="font-bold">
                    Success
                </p>
                <p>
            ...
            <form method="POST" class="w-full" action="{{ route('register') }}">
                ...
                <div class="flex items-center px-6 my-5">
                    <p class="text-sm">Already have an account? <a class="text-blue-700 underline"href="{{route('login')}}">Sign in</a></p>
                </div>
            </form>
        </div>
    </div>
    ...
    if (successMessage) {
        ...
        }, 4000);  // Adjust the time for how long the message stays visible
    }
});
